...
local function isAdmin(src)
  ...
  if fw then
    ...
    if type(fn) == "function" then
      ...
      dprint(("[isAdmin] AzFW returned non-bool (colon=%s dot=%s)"):format(tostring(res1), tostring(res2)))
    else
      dprint("[isAdmin] AzFW export missing: isAdmin")
    end
  end
...
local function doMoneyOp(adminSrc, op, target, amount, extra)
  ...
  if op == "add" then
    ...
    local ok, err = callMoney("addMoney", target, amount)
    if not ok then return false, err end

  elseif op == "deduct" then
    if amount == 0 then return false, "Missing amount" end
    local ok, err = callMoney("deductMoney", target, amount)
    if not ok then return false, err end
...
lib.callback.register("adminmenu:getReports", function(src)
  ...
  local out = {}
  for id, r in pairs(Reports or {}) do
